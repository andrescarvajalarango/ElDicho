...
export async function POST() {
  ...
  // Create dichos with staggered dates
  const now = Date.now();
  for (let i = 0; i < DICHOS.length; i++) {
    const dicho = DICHOS[i];
    const hoursAgo = i * 3 + Math.floor(Math.random() * 5);
    const createdAt = new Date(now - hoursAgo * 60 * 60 * 1000);
    await prisma.dicho.create({
      data: {
        text: dicho.text,
        meaning: dicho.meaning || null,
        author: dicho.author || null,
        isAnonymous: dicho.isAnonymous || false,
        userId: dicho.userId,
        departamentoId: deptMap[dicho.deptCode],
        createdAt,
      },
    });
  }
  ...
  const allDichos = await prisma.dicho.findMany();
  const comments = [
  ...
  for (const dicho of allDichos) {
    ...
    if (Math.random() > 0.4) {
      const commentUser =
        shuffledUsers[Math.floor(Math.random() * shuffledUsers.length)];
